...
async function fillForm(title, descText, tags) {
  ...
  if(tags&&tags.length>0){var n=normalizeTags(tags);var bare=n.map(function(t){return t.indexOf("#")===0?t.slice(1):t});var s=["input[placeholder*=\"标签\"]","input[placeholder*=\"tag\"]"];var ok=false;for(var i=0;i<s.length;i++){var el=document.querySelector(s[i]);if(el&&setNativeValue(el,bare.join(","))){el.dispatchEvent(new KeyboardEvent("keydown",{key:"Enter",bubbles:true}));await sleep(200);ok=true;logOk("Tags: OK");break}}if(!ok){var ds=["textarea","div[contenteditable=true]","[contenteditable=\"true\"]"];for(var i=0;i<ds.length;i++){var el=document.querySelector(ds[i]);if(!el)continue;var cur=el.tagName==="TEXTAREA"?el.value:el.innerText;if(cur){var nv=cur+"\n"+n.join(" ");if(el.tagName==="TEXTAREA"){setNativeValue(el,nv)}else{setCE(el,nv)};logOk("Tags: appended");ok=true;break}}}if(!ok)logFail("Tags: not filled")}
  logInfo("Form done")
}
...
async function processPublish(data) {
  ...
  var s=["video[src]","video","[class*=upload-success]","[class*=success]"]
