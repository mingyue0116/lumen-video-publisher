...
async function fillForm(title: string, descText: string): Promise<void> {
  ...
  if (descText) {
    ...
    for (var s = 0; s < ds.length; s++) { var el = document.querySelector(ds[s]) as HTMLElement; if (!el) continue; if (el.tagName === "TEXTAREA") { if (setNativeValue(el as HTMLTextAreaElement, descText)) { log("Description: OK"); break } } else if (el.getAttribute("contenteditable") === "true") { if (setContentEditable(el, descText)) { log("Description: OK"); break } } }
  }
