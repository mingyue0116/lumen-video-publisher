...
async function fillForm(title, descText, tags) {
  logInfo("Filling douyin...")
  if (title) {
    var s=["input[placeholder*=\"标题\"]","input[placeholder*=\"title\"]","input:not([type=file]):not([type=hidden])"]
    for(var i=0;i<s.length;i++){var el=document.querySelector(s[i]);if(el&&el.type!=="file"&&el.type!=="hidden"){if(setNativeValue(el,title)){logOk("Title: OK");break}}}
  }
  if (descText) {
    var s=["textarea[placeholder*=\"简介\"]","textarea[placeholder*=\"描述\"]","div[contenteditable=true]","[contenteditable=\"true\"]","textarea"]
    for(var i=0;i<s.length;i++){var el=document.querySelector(s[i]);if(!el)continue;if(el.tagName==="TEXTAREA"){if(setNativeValue(el,descText)){logOk("Desc: OK");break}}else if(el.getAttribute("contenteditable")==="true"){if(setCE(el,descText)){logOk("Desc: OK");break}}}
  }
  if (tags&&tags.length>0) {
    var n=normalizeTags(tags),tagStr=n.join(" ")
    var s=["input[placeholder*=\"话题\"]","input[placeholder*=\"tag\"]"]
    var ok=false
    for(var i=0;i<s.length;i++){var el=document.querySelector(s[i]);if(el&&setNativeValue(el,tagStr)){el.dispatchEvent(new KeyboardEvent("keydown",{key:"Enter",bubbles:true}));ok=true;logOk("Tags: "+tagStr);break}}
    if(!ok){var ds=["textarea","div[contenteditable=true]","[contenteditable=\"true\"]"];for(var i=0;i<ds.length;i++){var el=document.querySelector(ds[i]);if(!el)continue;var cur=el.tagName==="TEXTAREA"?el.value:el.innerText;if(cur){var nv=cur+"\n"+tagStr;if(el.tagName==="TEXTAREA"){setNativeValue(el,nv)}else{setCE(el,nv)};logOk("Tags: appended");ok=true;break}}}
...
async function processPublish(data) {
  ...
  var s=["video[src]","video","[class*=upload-success]","[class*=video-preview]","[class*=percent]"]
