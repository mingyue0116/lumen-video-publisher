...
async function fillForm(title: string, descText: string): Promise<void> {
  log("Filling form...")
  var titleSelectors = ["input[placeholder*=\"\\u6807\\u9898\"]", "input[placeholder*=\"title\"]", "input[placeholder*=\"\\u8f93\\u5165\\u89c6\\u9891\"]", "input:not([type=file]):not([type=hidden])"]
  if (title) {
    for (var s = 0; s < titleSelectors.length; s++) {
      var el = document.querySelector(titleSelectors[s]) as HTMLInputElement
      if (el && el.type !== "file" && el.type !== "hidden") { if (setNativeValue(el, title)) { log("Title: OK"); break } }
    }
  }
  if (descText) {
    var descSels = ["div[contenteditable=true]", "[contenteditable=\"true\"]", "div[role=\"textbox\"]", "textarea"]
    for (var s = 0; s < descSels.length; s++) {
      var el = document.querySelector(descSels[s]) as HTMLElement
      if (!el) continue
      if (el.getAttribute("contenteditable") === "true") { if (setContentEditable(el, descText)) { log("Description: OK"); break } }
      else if (el.tagName === "TEXTAREA") { if (setNativeValue(el as HTMLTextAreaElement, descText)) { log("Description: OK"); break } }
    }
  }
