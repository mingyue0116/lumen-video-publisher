...
var adapter = {
  ...
  findDescInput: function () {
    return deepQuerySelector('textarea[placeholder*="正文"], textarea[placeholder*="描述"], div[contenteditable=true], [contenteditable="true"]')
  },
...
// ===== Fill Form (Xiaohongshu-specific) =====
async function fillForm(title: string, descText: string, tags: string[]) {
  ...
  // Description (Xiaohongshu uses contenteditable for main text)
  if (descText) {
    var s = [
  ...
  // Tags (Xiaohongshu uses #tag format in the content)
  if (tags && tags.length > 0) {
    var n = normalizeTags(tags)
    var ts = n.join(" ")
    // Try dedicated tag input
