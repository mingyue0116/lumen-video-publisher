...
function getWujieRoot(): Document | ShadowRoot {
  var app = document.querySelector("wujie-app")
  if (app && (app as any).shadowRoot) {
    var sr = (app as any).shadowRoot; var iframe = sr.querySelector("iframe")
    if (iframe && (iframe as any).contentDocument) { return (iframe as any).contentDocument }
    return sr
  }
  return document
}
function wq(selector: string): Element | null {
  var root = getWujieRoot(); try { var el = root.querySelector(selector); if (el) return el } catch(e) {}
  return document.querySelector(selector)
}
function wqAll(selector: string): NodeListOf<Element> {
  var root = getWujieRoot(); try { var list = root.querySelectorAll(selector); if (list.length > 0) return list } catch(e) {}
  return document.querySelectorAll(selector)
}
async function fillForm(title: string, descText: string): Promise<void> {
  log("Filling form (Wujie)...")
  if (title) {
    var inputs = wqAll("input")
    for (var i = 0; i < inputs.length; i++) {
      var inp = inputs[i] as HTMLInputElement
      var ph = (inp.placeholder || "").toLowerCase()
      if ((ph.indexOf("\\u6807\\u9898") >= 0 || ph.indexOf("title") >= 0 || ph.indexOf("\\u6982\\u62ec") >= 0) && inp.type !== "file" && inp.type !== "hidden") {
        if (setNativeValue(inp, title)) { log("Title: OK"); break }
      }
    }
  }
  if (descText) {
    var editors = wqAll("div[contenteditable=true]")
    for (var i = 0; i < editors.length; i++) { if (setContentEditable(editors[i] as HTMLElement, descText)) { log("Description: OK"); break } }
  }
